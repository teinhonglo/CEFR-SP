...
from util import read_corpus, CEFRDataset, convert_numeral_to_eight_levels
from model_base_w2v import LevelEstimaterW2vBase as LevelEstimaterBase
import losses
...
class LevelEstimaterClassification(LevelEstimaterBase):
    def __init__(self, corpus_path, test_corpus_path, pretrained_model, problem_type, with_ib, with_loss_weight,
        ...
        if self.problem_type == "regression":
            self.slv_classifier = nn.Linear(self.wav_lm.config.hidden_size, 1)
            self.loss_fct = nn.MSELoss()
        else:
            if args.use_prediction_head:
                self.slv_classifier = PredictionHead(self.wav_lm.config.hidden_size, args)
            else:
                self.slv_classifier = nn.Linear(self.wav_lm.config.hidden_size, self.CEFR_lvs)
...
class LevelEstimaterContrastive(LevelEstimaterBase):
    def __init__(self, corpus_path, test_corpus_path, pretrained_model, problem_type, with_ib, with_loss_weight,
        ...
        self.prototype = nn.Embedding(self.CEFR_lvs * self.num_prototypes, self.wav_lm.config.hidden_size)
    ...
    def on_train_start(self) -> None:
        # Init with Wav2Vec embeddings
        epcilon = 1.0e-6
        labels = []
        prototype_initials = torch.full((self.CEFR_lvs, self.wav_lm.config.hidden_size), fill_value=epcilon).to(self.device)
